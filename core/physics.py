...
class PhysicsEngine:
    ...
    def _aggregate_network_load(self, world_state: WorldState):
        ...
        for router in routers:
            ...
            try:
                current_load = world_state.get_attribute_value(
                    router.id, "network_load"
                )
                new_load = max(
                    0.0, min(100.0, current_load + total_load)
                )  # Clamp 0-100
                world_state.set_attribute_value(router.id, "network_load", new_load)

                # Update connected devices count
                world_state.set_attribute_value(
                    router.id, "connected_devices_count", len(active_devices)
                )
    ...
    def _eval_condition(
        self, entity: Entity, condition: str, world_state: WorldState
    ) -> bool:
