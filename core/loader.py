...
class DataLoader:
    def load(self, file_path: str | Path) -> tuple[WorldState, list[Physics]] | None:
        path = Path(file_path)
        if not path.exists():
            logger.error("error", "file_not_found", path=str(path))
            return None
        ...
        try:
            ...
            world_state = WorldState(
                entities=entity_map, global_attributes=raw_global_attrs
            )

            return world_state, physics_data

        except (ValidationError, yaml.YAMLError) as e:
            logger.error("error", "loader_failed", file=str(path), error=str(e))
            return None
